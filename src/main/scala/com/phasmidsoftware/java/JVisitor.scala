...
object JVisitor {
   ...
   */
  def create[X](message: JMessage, journal: com.phasmidsoftware.visitor.Appendable[X]): JVisitor[X] =
  ...
  def createPreQueue[X]: JVisitor[X] =
    create(JMessage.PRE, QueueJournal.empty[X])
  ...
  def createPostQueue[X]: JVisitor[X] =
    create(JMessage.POST, QueueJournal.empty[X])
  ...
  def createPreStack[X]: JVisitor[X] =
    create(JMessage.PRE, ListJournal.empty[X])
  ...
  def createPostStack[X]: JVisitor[X] =
    create(JMessage.POST, ListJournal.empty[X])
